...
class Background():
	def CreateCorrectFolderStructure(self):
	...
	def CreateComponentDummies(self):
...
class Happy_DeploymentPomIsRead(unittest.TestCase):
	...
	def test_given_CorrectFolderStructureAndFilesAreCreated(self):
		Background().CreateCorrectFolderStructure()
		Background().CreateComponentDummies()

	def test_and_ValidPomFileIsCreated(self):
	...
	def test_and_noMissingDependenciesAreFoundUnderSynapseConfigFolder(self):
		self.assertEqual(HelperUtil().findMissingDependencies(["MySequence", "MyEndpint", "MyApi", "MyProxy", "MyTask"], artifactXmlFolder), [])
